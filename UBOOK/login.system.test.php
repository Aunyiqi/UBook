<?php
/**
 * MainMenu.php - UBook Dashboard (Final Working Version)
 * 
 * All features: popular venues, description toggle, comments with working likes,
 * AI chat (DeepSeek, Malaysia time), booking cart, reviews.
 * 
 * Fixed: No "use PHPMailer" statements to avoid naming conflicts.
 */
...
// ---------------------- PHPMailer autoload (no "use" statements to avoid conflict) ----------------------
...
// ==================== EMAIL SENDING (auto to logged-in user, no "use" statements) ====================
function sendBookingConfirmation($userId, $studentName, $items) {
    ...
    // Use fully qualified class names – no "use" statement needed
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // --- Get comments for a venue (enhanced) ---
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Enhanced Comments Toggle Button (without badge) */
        .toggle-comments-btn {
...
<header>
    ...
    <div class="nav-links">
        <a href="MainMenu.php" class="active">Home</a>
...
<footer>
    <div class="footer-content">
        ...
        <div class="footer-column"><h3>Quick Links</h3><div class="footer-links"><a href="MainMenu.php">Home</a><a href="venue.php">Venues</a><a href="Community.php">Community</a></div></div>
    ...
    <div class="copyright"><p>&copy; 2025 UBook – Venue Booking System. AI Assistant powered by DeepSeek</p></div>
</footer>
...
function addDaysFromToday(days) { const d = new Date(); d.setDate(d.getDate() + days); return d.toISOString().split('T')[0]; }
...
function addBotMessage(text, isHtml = false) { const chatBody = document.getElementById('chat-body'); const msgDiv = document.createElement('div'); msgDiv.classList.add('bot-message'); if (isHtml) msgDiv.innerHTML = text; else msgDiv.textContent = text; chatBody.appendChild(msgDiv); scrollToBottom(); saveChatHistory(); }
function addUserMessage(text) { const chatBody = document.getElementById('chat-body'); const msgDiv = document.createElement('div'); msgDiv.classList.add('user-message'); msgDiv.textContent = text; chatBody.appendChild(msgDiv); scrollToBottom(); saveChatHistory(); }
...
function toggleChat() { 
    ...
    if (popup.style.display === 'flex') { 
        ...
        document.getElementById('current-time').textContent = "<?php echo date('h:i A'); ?>";
...
function renderChatHistoryFromMessages() { const chatBody = document.getElementById('chat-body'); const existing = chatBody.querySelectorAll('.bot-message, .user-message'); existing.forEach(m => m.remove()); for (let msg of chatMessages) { if (msg.role === 'system') continue; const div = document.createElement('div'); div.classList.add(msg.role === 'user' ? 'user-message' : 'bot-message'); div.textContent = msg.content; chatBody.appendChild(div); } scrollToBottom(); }
...
// ==================== ENHANCED COMMENTS & LIKES (working with event delegation) ====================
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // ========== EVENT DELEGATION FOR LIKE AND DELETE BUTTONS (fixes dynamic content) ==========
    document.body.addEventListener('click', async function(e) {
        ...
        if (likeBtn) {
            ...
            if (!commentId) return;
            try {
                const resp = await fetch(window.location.href, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'toggle_like', comment_id: commentId }) });
                const data = await resp.json();
                if (data.success) {
                ...
                console.error(err);
                showToast('Error toggling like', 3000, true);
            }
            return;
        }
        ...
        if (deleteBtn) {
            ...
            if (!commentId) return;
            try {
                const resp = await fetch(window.location.href, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'delete_comment', comment_id: commentId }) });
                const data = await resp.json();
                if (data.success) {
                ...
                console.error(err);
                showToast('Error deleting comment', 3000, true);
            }
            return;
        }
    });
